...
function checkConnection(prevChord, currentChord, brokenRulesCounter){
    ...
    if((prevChord.harmonicFunction.functionName === Consts.FUNCTION_NAMES.DOMINANT
        ...
        if(prevChord.harmonicFunction.functionName === Consts.FUNCTION_NAMES.DOMINANT && currentChord.harmonicFunction.functionName === Consts.FUNCTION_NAMES.TONIC
            && currentChordFunctionTemp.degree - prevChord.harmonicFunction.degree === 1) {
            couldHaveDouble3 = true;
            var dominantVoiceWith3 = -1;
            for (var i = 0; i < 4; i++) {
                if (prevChord.notes[i].baseChordComponentEquals("3")) {
    ...
    if(prevChord.harmonicFunction.functionName === Consts.FUNCTION_NAMES.DOMINANT
        && prevChord.harmonicFunction.mode === Consts.MODE.MAJOR && currentChordFunctionTemp.functionName === Consts.FUNCTION_NAMES.SUBDOMINANT){
...
function checkAllRules(prevPrevChord, prevChord, currentChord, brokenRulesCounter){
