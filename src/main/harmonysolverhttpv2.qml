...
MuseScore {
    ...
    property var port: "9000"
    property var harmonicsEndpoint: "/harmonytools/harmonics"
    property var validatorEndpoint: "/harmonytools/validator"
    property var bassEndpoint: "/harmonytools/bass"
    property var sopranoEndpoint: "/harmonytools/soprano"
    property var parserEndpoint: "/harmonytools/parser"
...
//todo do osobnej funkcji
    ...
    function fill_score_with_solution(solution, durations) {
        ...
        for (var i = 0; i < solution.chords.length; i++) {
            ...
            var nominator = curChord.duration*1024
            var denominator = 1024
            cursor.setDuration(nominator, denominator)
                        ...
                        onClicked: {
                            ...
                            if (input_text === undefined || input_text === "") {
                              emptyExerciseDialog.open()
                                ...
                                preReq.open("POST", getURL(parserEndpoint))
                                preReq.setRequestHeader("Content-Type", "application/json")
                                ...
                                preReq.onreadystatechange = function(){
                                    if (preReq.readyState == XMLHttpRequest.DONE) {
                                        if (preReq.status === 200) {
                                            ...
                                            req.send(
                                                JSON.stringify({
                                                    "exercise": JSON.parse(preReq.response),
